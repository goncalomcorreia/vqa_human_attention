...
class DataProvisionAttVqaWithMaps(DataProvisionAttVqaTest):
    def __init__(self, data_folder, feature_file, maps_data_folder, rng = None, state = None, n_shuffles = None):
        ...
        self._att_maps_imids = OrderedDict()
        maps = h5py.File(os.path.join(maps_data_folder,'map_dist_196_salicon.h5'), 'r')
        att_maps = np.array(maps['label'])
        maps.close()
        for maps_split in ['train', 'val']:
            with open(os.path.join(maps_data_folder, maps_split) + '_salicon.pkl') as f:
                self._att_maps_imids[maps_split] = pkl.load(f)
            if maps_split == 'train':
                self._att_maps[maps_split] = att_maps[:len(self._att_maps_imids[maps_split])]
            else:
                self._att_maps[maps_split] = att_maps[len(self._att_maps_imids[maps_split]):]

        self._map_label = OrderedDict()
        imids_with_maps = self._att_maps_imids['train'] + self._att_maps_imids['val']
        for split in self._splits:
            import ipdb; ipdb.set_trace()
            map_idx = np.where(np.in1d(self._image_id[split], imids_with_maps))[0]
            ...
            sort = np.argsort(imids_with_maps)
            rank = np.searchsorted(imids_with_maps, self._image_id[split], sorter=sort)
    ...
    def check(self):
        for i in xrange(len(self._image_id['train'])):
            im_id = self._image_id['train'][i]
            ind_orig = self._att_maps_imids['train'].index(im_id)
            booli = np.isclose(self._att_maps['train'][ind_orig], self._map_label['train'][i])
            if booli.sum()!=196:
                return False
        return True
    ...
    def get_map_from_qid(self, qid):
        for maps_split in ['train', 'val']:
            if qid in self._att_maps_imids[maps_split]:
                idx = self._att_maps_imids[maps_split].index(qid)
