...
def init_params(options):
    ...
    params = init_fflayer(params, n_filter, n_attention, options,
                          prefix='image_mlp_salicon')
    params = init_fflayer(params, n_attention, 1, options,
                          prefix='image_mlp_salicon_att')
...
def build_model(shared_params, options):
    ...
    # SALICON Layers
    image_feat_att_salicon = fflayer(shared_params, image_feat_down, options,
                              prefix='image_mlp_salicon',
                              act_func=options.get('image_mlp_act',
                                                   'tanh'))
    image_mlp_salicon_att = fflayer(shared_params,
                                    image_feat_att_salicon, options,
                                    prefix='image_mlp_salicon_att',
                                    act_func=options.get(
                                        'combined_att_mlp_act',
                                        'tanh'))
    prob_attention_salicon = T.nnet.softmax(image_mlp_salicon_att[:, :, 0])
    ...
    image_feat_ave_salicon = (prob_attention_salicon[:, :, None] * image_feat_down).sum(axis=1)
    ...
    pool_feat_attention_1 = fflayer(shared_params, pool_feat, options,
                                    ...
                                    act_func=options.get('sent_att_mlp_act',
                                                         'tanh'))
    combined_feat_attention_1 = image_feat_ave_salicon + \
