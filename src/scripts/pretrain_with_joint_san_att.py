...
options['model_name'] = 'pretrain_with_joint'
...
##############################
    ...
    for itr in xrange(beggining_itr, max_iters + 1):
        ...
        if (itr % eval_interval_in_iters) == 0 or (itr == max_iters):
            train_learn_curve_err_map = np.append(train_learn_curve_err_map, cost)
...
##############################
    ...
    # Use best parameters of Pre-Training
    cpu_to_shared(best_param, shared_params)
